...
int JugadorMaquina::casillasEnDireccion(Direccion dir, int k, int jug){
    ...
    while(dentroBordes(dir.x+mult*Dirs[k].x,dir.y+mult*Dirs[k].y)  && tablero->obtenerCasilla(dir.x+mult*Dirs[k].x,dir.y+mult*Dirs[k].y) == jug){
        if(primera == true && dentroBordes(dir.x+Dirs[k+4].x,dir.y+Dirs[k+4].y) && tablero->obtenerCasilla(dir.x+Dirs[k+4].x,dir.y+Dirs[k+4].y) != jug){
            break;
        }
        primera = false;
        mult++;
            ...
            if(tablero->obtenerCasilla(dir.x,dir.y) == jug){
                ...
                //cout << "Ficha en direccion: " << dir.x << ", " << dir.y << "    ficha sola " << endl;
                    ...
                    for(int k = 0; k < 4; k++){
                        ...
                        int dir2 = casillasEnDireccion(dir,k+4,jug);
                        int pos1 = posibilidadesEnDireccion(dir,k,dir1);
                        int pos2 = posibilidadesEnDireccion(dir,k,dir2);
